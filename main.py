...
def run_cipher_suites_scan(hostname: str, port: int) -> List[CipherSuitesScanResult]:
    ...
    server_scan_req = ServerScanRequest(
        server_info=server_info, scan_commands=CIPHER_SUITES_SCAN_CMDS
    )
    scanner = Scanner()
    scanner.queue_scan(server_scan_req)

    # Then retrieve the results
    scan_results: List[CipherSuitesScanResult] = []
    for server_scan_result in scanner.get_results():
        for cipher_suite_scan_cmd in CIPHER_SUITES_SCAN_CMDS:
            scan_results.append(
                server_scan_result.scan_commands_results[cipher_suite_scan_cmd]
            )
...
class ScanResultPage:
    def on_post(self, request: falcon.Request, response: falcon.Response) -> None:
        ...
        response.body = result_template.render(
            hostname=hostname,
            port=port,
            cipher_suites_scan_results=cipher_suites_scan_results,
        )
        response.content_type = falcon.MEDIA_HTML
